...
const UpdateStore = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const hostingBannerImg = await axiosPublic.post(
      image_hosting_api,
      bannerImageFile,
      {
        headers: {
    ...
    const hostingIconImg = await axiosPublic.post(
      image_hosting_api,
      iconImageFile,
      {
        headers: {
    ...
    if (hostingBannerImg.data.success && hostingIconImg.data.success) {
      ...
      const storeItem = await axiosPublic.patch(
        `/stores/${store._id}`,
        newStoreItem
      );
      if (storeItem) {
  ...
  return (
    <div className="w-full md:w-[870px] px-4 mx-auto">
      <h2 className="text-2xl font-semibold my-4">
        Update the <span className="text-blue-500">Store</span>
      </h2>
